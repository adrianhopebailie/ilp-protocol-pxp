...
interface IlpTransportOptions {
  ...
  /**
   * The initial handlers to use for incoming requests. Copied into the new `IlpTransport`'s `handlers` Map.
   */
...
export class IlpTransport extends EventEmitter implements IlpEndpoint {
  ...
  constructor (stream: Duplex, options?: IlpTransportOptions) {
    ...
    if (options && options.handlers) {
      options.handlers.forEach((handler, key) => {
  ...
  /**
   * Map of `RequestHandler`s that handle incoming requests.
   *
   * The map is indexed by ILP Address with the value '*' serving as a catch-all.
   */
